<?php 
    ...
    $data = mysqli_query($conn, "SELECT * FROM ticket WHERE id_ticket = '".$_GET['idh']."' ");
  ...
  <nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="border-bottom: 2px solid #7C924E; background-color: white;">
    ...
    <div class="container" style="margin-top: 75px; margin-bottom: 50px;">
      <div class="main-body">
        <?php
            $id = $data2->id_ticket;
            $ticket = mysqli_query($conn, "SELECT image FROM ticket WHERE id_ticket = $id");
            $ambil = mysqli_fetch_assoc($ticket);
          ?>
        <nav class="navbar navbar-expand-lg navcolor" style="border-bottom: 2px solid #7C924E;">
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
                    ...
                    <button onclick="location.href='editconcert.php?id=<?php echo $data2->id_ticket ?>'" class="btn btn-primary" name="edit" style="display: inline; margin: 0 10px 0 20px;">Edit</button>
                    <button onclick="del_concert()" class="btn btn-danger" name="delete" style="display: inline; margin: 0 20px 0 10px;">Delete</button>
                    <script>
                    function del_concert() {
                            if (confirm("Are you sure to delete this ticket concert?")) {
                            window.location.href='../hapus/hapusalbum.php?idt=<?php echo $data2->id_ticket ?>';
                            } else {
                            window.location.href='';
                            }
                        }
        ...
        <div class="container container px-4 py-5">
          ...
          <div class="row ">
              ...
              <h3 style="font-weight: bold; font-size: 30px;">Ticket Concert</h3>
          ...
          <div class="row" style="font-size: 20px;">
              ...
              <form action="payment.php" method= "GET">
                  ...
                  <input type="number" class="form-control" name="Quantity" id="Quantity"" placeholder="1" min="1" max="1" required>
                  <p style="font-size: 16px; color: grey;">p.s. Maximum quantity that can be bought are 1 (one) for each person!!!</p>
                  <?php
                      $ticket = mysqli_query($conn, "SELECT * FROM ticket ORDER BY id_ticket DESC");
                      $ambil = mysqli_fetch_array($ticket);
                  ?>
                  <button type="button" class="btn btn-sm" style="background-color:#7C924E; color:white; font-size:20px; width: 150px; height: 50px; margin-bottom: 30px" onclick="window.location.href='payment.php?idp=<?php echo $data2->id_ticket ?>&Quantity='+document.getElementById('Quantity').value;">Buy</button>
              </form>
